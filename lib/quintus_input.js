      ...
      if(p.ignoreControls === undefined || !p.ignoreControls) {
        ...
        if(Q.inputs['left']) {
          ...
          if(collision && p.landed > 0) {
            ...
            console.log("NORMAL X " + collision.normalX)
            console.log("NORMAL Y " + collision.normalY)
          } else {
            p.vx = -p.speed;
          }
          ...
          //MODIFICACION SOBRE QUINTUS PARA MANTENER INERCIA
          /*Si se ha dejado de pulsar disminuye la velocidad gradualmente*/
          if(p.vx > 0){
            p.vx -= 5;
          }
          else if(p.vx < 0){
